...
from ray.rllib.algorithms.mu_zero.mu_zero_policy import MuZeroPolicy
from ray.rllib.algorithms.mu_zero.mcts import MCTS
from ray.rllib.algorithms.mu_zero.ranked_rewards import get_r2_env_wrapper
...
class MuZeroDefaultCallbacks(DefaultCallbacks):
    """MuZero callbacks.
...
class MuZeroConfig(AlgorithmConfig):
    """Defines a configuration class from which an MuZero Algorithm can be built.
    ...
    def __init__(self, algo_class=None):
        ...
        super().__init__(algo_class=algo_class or MuZero)

        # fmt: off
        # __sphinx_doc_begin__
        # MuZero specific config settings:
        ...
        # Override some of AlgorithmConfig's default values with MuZero-specific
        # values.
        self.framework_str = "torch"
        self.callbacks_class = MuZeroDefaultCallbacks
        self.lr = 5e-5
...
class MuZeroPolicyWrapperClass(MuZeroPolicy):
...
class MuZero(Algorithm):
    ...
    def get_default_policy_class(self, config: AlgorithmConfigDict) -> Type[Policy]:
        return MuZeroPolicyWrapperClass
    ...
    def execution_plan(
        ...
        assert (
            len(kwargs) == 0
        ), "Mu zero execution_plan does NOT take any additional parameters"
...
# Deprecated: Use ray.rllib.algorithms.alpha_zero.MuZeroConfig instead!
class _deprecated_default_config(dict):
    def __init__(self):
        super().__init__(MuZeroConfig().to_dict())

    @Deprecated(
        old="ray.rllib.algorithms.alpha_zero.alpha_zero.DEFAULT_CONFIG",
        new="ray.rllib.algorithms.alpha_zero.alpha_zero.MuZeroConfig(...)",
        error=False,
    )
